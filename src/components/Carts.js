    ...
    <>
      <section className="md:flex md:justify-center  w-4/5 mx-auto md:my-5 fmd:flex-wrap gap-5">
            ...
            return (
              ...
              >
                ...
                <div className="md:w-1/3 text-md ">
                  ...
                  <p className="">Rs. {(parseInt(item?.price) / 100).toFixed(2)} </p>
                  <p className="my-2 font-bold text-lg">
                    Total price:{" "}
                    {((item.itemCount * (parseInt(item?.price) / 100)).toFixed(2))}
        ...
        {cartItems.length === 0 ? null : (
           ...
           Total price: Rs.{" "}
           {cartItems.length && cartItems.map((item)=> (item.itemCount * (parseInt(item?.price)) / 100))
